                          ...
                          const Sphere *sphere)
{
    ...
    __m128d D_dup = _mm_permute_pd(D, 1);
    err1 = _mm_cmpneq_pd(D, D_dup);
    if (_mm_movemask_pd(err1))	errors++;
                         ...
                         const Plane *plane)
{
    ...
    __m128d v_dup = _mm_permute_pd(v, 1);
    err1 = _mm_cmpneq_pd(v, v_dup);
    if (_mm_movemask_pd(err1))	errors++;
    ...
    __m128d dp_dup = _mm_permute_pd(dp, 1);
    err1 = _mm_cmpneq_pd(dp, dp_dup);
    if (_mm_movemask_pd(err1))	errors++;

    __m128d t2 = _mm_and_pd(cond1, _mm_div_pd(_MM_NEGATE_(_mm_add_pd(dp, d)), v));
    __m128d t2_dup = _mm_permute_pd(t2, 1);
    err1 = _mm_cmpneq_pd(t2, t2_dup);
    if (_mm_movemask_pd(err1))	errors++;
...
void ambient_occlusion(vec *col, const Isect *isect)
{
    ...
    __m128d occlusionx2 = _mm_set1_pd(0.0);
    __m128d err1;
    
    for (j = 0; j < ntheta; j++) {
        
        double __attribute__ ((__aligned__(16))) rand1[nphi];
        double __attribute__ ((__aligned__(16))) rand2[nphi];
	    ...
	    __m128d rnd1_low = _mm_set1_pd(rand1[i]);
            __m128d theta = _mm_sqrt_pd(rnd1_low);
	    __m128d theta_dup = _mm_permute_pd(theta, 1);
	    err1 = _mm_cmpneq_pd(theta, theta_dup);
	    if (_mm_movemask_pd(err1))
		errors++;

	    __m128d rnd2_low = _mm_set1_pd(rand2[i]);
            __m128d phi = _mm_mul_pd(_mm_set1_pd(2.0 * M_PI), rnd2_low);
            __m128d sinphi; // = _mm_sin_pd(phi);
            __m128d cosphi; // = _mm_cos_pd(phi);
            sincos_pd(phi, &sinphi, &cosphi);
            __m128d x = _mm_mul_pd(cosphi, theta);
	    __m128d x_dup = _mm_permute_pd(x, 1);
	    err1 = _mm_cmpneq_pd(x, x_dup);
	    if (_mm_movemask_pd(err1))	errors++;

            __m128d y = _mm_mul_pd(sinphi, theta);
	    __m128d y_dup = _mm_permute_pd(y, 1);
	    err1 = _mm_cmpneq_pd(y, y_dup);
	    if (_mm_movemask_pd(err1))	errors++;

            __m128d z = _mm_sqrt_pd(_mm_sub_pd(_mm_set1_pd(1.0), _mm_mul_pd(theta, theta)));
	    __m128d z_dup = _mm_permute_pd(z, 1);
	    err1 = _mm_cmpneq_pd(z, z_dup);
	    if (_mm_movemask_pd(err1))	errors++;
	    ...
	    __m128d dirx_dup = _mm_permute_pd(dirx, 1);
	    err1 = _mm_cmpneq_pd(dirx, dirx_dup);
	    if (_mm_movemask_pd(err1))	errors++;
	   ...
	   __m128d diry_dup = _mm_permute_pd(diry, 1);
	   err1 = _mm_cmpneq_pd(diry, diry_dup);
	   if (_mm_movemask_pd(err1))	errors++;
	    ...
	    __m128d dirz_dup = _mm_permute_pd(dirz, 1);
	    err1 = _mm_cmpneq_pd(dirz, dirz_dup);
	    if (_mm_movemask_pd(err1))	errors++;
	    ...
	    __m128d orgx = _mm_set1_pd(p.x);
            ...
            occlusionx2 = _mm_add_pd(occlusionx2, _mm_and_pd(hit, _mm_set1_pd(1.0f)));
            __m128d occ_dup = _mm_permute_pd(occlusionx2, 1);
	    err1 = _mm_cmpneq_pd(occlusionx2, occ_dup);
            if (_mm_movemask_pd(err1))	errors++;
        }
    }
    
    double __attribute__ ((__aligned__(16))) occlusionTmp[2];
    _mm_store_pd(occlusionTmp, occlusionx2);
    occlusion = occlusionTmp[0]; 
