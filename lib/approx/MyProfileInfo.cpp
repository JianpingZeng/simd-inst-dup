//===- MyProfileInfo.cpp - Profile Info Interface ---------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the abstract MyProfileInfo interface, and the default
...
namespace llvm {
  //template<> char MyProfileInfoT<Function,BasicBlock>::ID = 0;
}

// Register the MyProfileInfo interface, providing a nice name to refer to.
//INITIALIZE_ANALYSIS_GROUP(MyProfileInfo, "Profile Information", NoMyProfileInfo)

namespace llvm {

template <>
MyProfileInfoT<MachineFunction, MachineBasicBlock>::MyProfileInfoT() {}
template <>
MyProfileInfoT<MachineFunction, MachineBasicBlock>::~MyProfileInfoT() {}

template <>
MyProfileInfoT<Function, BasicBlock>::MyProfileInfoT() {
  MachineProfile = 0;
}
template <>
MyProfileInfoT<Function, BasicBlock>::~MyProfileInfoT() {
...
//template<>
//char MyProfileInfoT<MachineFunction, MachineBasicBlock>::ID = 0;

template<>
const double MyProfileInfoT<Function,BasicBlock>::MissingValue = -1;

template<> const
double MyProfileInfoT<MachineFunction, MachineBasicBlock>::MissingValue = -1;

template<> double
MyProfileInfoT<Function,BasicBlock>::getExecutionCount(const BasicBlock *BB) {
...
template<>
double MyProfileInfoT<MachineFunction, MachineBasicBlock>::
...
template<>
double MyProfileInfoT<Function,BasicBlock>::getExecutionCount(const Function *F) {
...
template<>
void MyProfileInfoT<Function,BasicBlock>::
...
template<>
void MyProfileInfoT<MachineFunction, MachineBasicBlock>::
...
template<>
void MyProfileInfoT<Function,BasicBlock>::addEdgeWeight(Edge e, double w) {
...
template<>
void MyProfileInfoT<Function,BasicBlock>::removeBlock(const BasicBlock *BB) {
  std::map<const Function*, BlockCounts>::iterator J =
    BlockInformation.find(BB->getParent());
...
template<>
void MyProfileInfoT<Function,BasicBlock>::removeEdge(Edge e) {
...
template<>
const BasicBlock *MyProfileInfoT<Function,BasicBlock>::
...
/// Splits an edge in the MyProfileInfo and redirects flow over NewBB.
...
template<>
void MyProfileInfoT<Function,BasicBlock>::splitEdge(const BasicBlock *FirstBB,
  ...
  if (!MergeIdenticalEdges) {
    ...
    if (getExecutionCount(NewBB) == MyProfileInfo::MissingValue) succ_count++;
  } else {
...
template<>
void MyProfileInfoT<Function,BasicBlock>::splitBlock(const BasicBlock *Old,
...
template<>
void MyProfileInfoT<Function,BasicBlock>::splitBlock(const BasicBlock *BB,
                                                   const BasicBlock* NewBB,
...
template<>
void MyProfileInfoT<Function,BasicBlock>::transfer(const Function *Old,
                                                 const Function *New) {
...
static double readEdgeOrRemember(MyProfileInfo::Edge edge, double w,
                                 MyProfileInfo::Edge &tocalc, unsigned &uncalc) {
  if (w == MyProfileInfo::MissingValue) {
    tocalc = edge;
    uncalc++;
    return 0;
  } else {
...
template<>
bool MyProfileInfoT<Function,BasicBlock>::
...
static void readEdge(MyProfileInfo *PI, MyProfileInfo::Edge e, double &calcw, std::set<MyProfileInfo::Edge> &misscount) {
...
template<>
bool MyProfileInfoT<Function,BasicBlock>::EstimateMissingEdges(const BasicBlock *BB) {
  double inWeight = 0;
...
template<>
void MyProfileInfoT<Function,BasicBlock>::repair(const Function *F) {
...
//===----------------------------------------------------------------------===//
//  NoProfile MyProfileInfo implementation
//

namespace {
  struct NoMyProfileInfo : public ImmutablePass, public MyProfileInfo {
    static char ID; // Class identification, replacement for typeinfo
    NoMyProfileInfo() : ImmutablePass(ID) {
      //initializeNoMyProfileInfoPass(*PassRegistry::getPassRegistry());
    }
    ...
    virtual const char *getPassName() const {
      return "NoMyProfileInfo";
    }
  };
}  // End of anonymous namespace

char NoMyProfileInfo::ID = 0;
// Register this pass...
//INITIALIZE_AG_PASS(NoMyProfileInfo, MyProfileInfo, "no-profile",
//"No Profile Information", false, true, true)
static RegisterPass<NoMyProfileInfo> X("my-no-profile", "No Profile Information");

//ImmutablePass *llvm::createNoMyProfileInfoPass() { return new NoMyProfileInfo(); }
