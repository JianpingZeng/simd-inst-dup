...
//===----------------------------------------------------------------------===//
//
// The MyProfileInfoLoader class is used to load and represent profiling
...
static unsigned AddCounts(unsigned A, unsigned B) {
  ...
  if (A == MyProfileInfoLoader::Uncounted) return B;
  if (B == MyProfileInfoLoader::Uncounted) return A;
  return A + B;
}
...
static void ReadProfilingBlock(const char *ToolName, FILE *F,
  ...
  if (Data.size() < NumEntries)
    Data.resize(NumEntries, MyProfileInfoLoader::Uncounted);
...
const unsigned MyProfileInfoLoader::Uncounted = ~0U;

// MyProfileInfoLoader ctor - Read the specified profiling data file, exiting the
// program if the file is invalid or broken.
//
MyProfileInfoLoader::MyProfileInfoLoader(const char *ToolName,
